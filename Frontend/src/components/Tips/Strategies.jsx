const Strategies = () => {
  ...
  return (
    <>
      <section className="mx-auto mb-24 grid max-w-7xl grid-cols-1 gap-8 px-6 sm:grid-cols-2 lg:grid-cols-4 items-start">
        {strategies.map((strategie, index) => (
          <div
            key={index}
            className="h-full rounded-2xl border border-slate-200 bg-white p-8 shadow-lg
                 transition-all duration-300 ease-out
                 hover:-translate-y-1 hover:shadow-2xl"
          >
            {/* TITLE */}
            <h3 className="mb-6 border-b-2 border-slate-200 pb-3 text-xl font-semibold text-indigo-600">
              {strategie.title}
            </h3>
