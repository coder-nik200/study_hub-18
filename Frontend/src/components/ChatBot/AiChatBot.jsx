...
function AiChatBot() {
  ...
  return (
    <>
      ...
      <div
        ref={chatWindowRef}
        className={`fixed bottom-22 right-6 w-[360px] bg-white rounded-xl shadow-xl flex flex-col overflow-hidden z-50 transition-all duration-300 ease-out
        ${
          isOpen
      ...
      >
        ...
        <div className="flex items-center gap-2 p-3 border-t border-gray-400">
          <input
            ...
            className="flex-1 px-3 py-2 text-sm border border-gray-400 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          <button
            onClick={sendMessage}
            disabled={loading}
