...
export default function ExpertDashboard() {
  ...
  const fetchTasks = async (page = currentPage) => {
    try {
      ...
      const res = await fetchExpertTasks(page);
  ...
  const overallStats = {
    ...
    totalStudents: new Set(
      allTasks.flatMap((t) => t.assignedTo?.map((s) => s._id) || []),
    ).size,
    avgCompletionRate:
      allTasks.length > 0
        ? Math.round(
        ...
        : 0,
    totalCompleted: allTasks.filter((t) => t.progress?.completionRate === 100)
      .length,
  ...
  const statusDistribution = [
    {
      label: "Completed",
      value: allTasks.reduce((sum, t) => sum + (t.progress?.completed || 0), 0),
    },
    {
      label: "In Progress",
      value: allTasks.reduce(
        (sum, t) => sum + (t.progress?.inProgress || 0),
        0,
      ),
    },
    {
      label: "Pending",
      value: allTasks.reduce((sum, t) => sum + (t.progress?.pending || 0), 0),
    },
  ];
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-indigo-50 via-white to-indigo-100 flex flex-col md:flex-row">
      ...
      <div className="flex-1 p-4 sm:p-6 md:p-10">
        ...
        {filter === "all" && searchTerm === "" && totalPages > 1 && (
