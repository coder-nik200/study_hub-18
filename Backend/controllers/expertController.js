...
const getTasksByExpert = async (req, res) => {
  try {
    ...
    // Total tasks count
    const totalTasks = await ExpertTask.countDocuments({ createdBy: expertId });
    ...
    const allTasks = await ExpertTask.find({ createdBy: expertId })
      .populate("assignedTo", "name email avatar")
      .sort({ createdAt: -1 });

    // Reusable progress function
    const addProgress = async (taskList) => {
      return Promise.all(
    ...
    const tasksWithProgress = await addProgress(tasks);
    const allTasksWithProgress = await addProgress(allTasks);
